...
class TestETHProvider(TestCase):
    ...
    def test_verify_address(self):
        ...
        self.assertEqual(
            data.AddressValidation(normalized_address="", display_address="", is_valid=False),
    ...
    def test_fill_unsigned_tx(self):
        ...
        fake_client = Mock(
            get_prices_per_unit_of_fee=Mock(
                return_value=data.PricesPerUnit(
                    normal=data.EstimatedTimeOnPrice(price=int(30 * 1e9)),
                    others=[
                        data.EstimatedTimeOnPrice(price=int(20 * 1e9)),
                        data.EstimatedTimeOnPrice(price=int(50 * 1e9)),
                    ],
                )
            ),
        ...
        with self.subTest("Empty data.UnsignedTx"):
            self.assertEqual(
                data.UnsignedTx(fee_limit=21000, fee_price_per_unit=int(30 * 1e9)),
        ...
        with self.subTest("Transfer ETH to external address with preset gas price"):
            self.assertEqual(
                data.UnsignedTx(
                    inputs=[data.TransactionInput(address=external_address_a, value=21)],
                    outputs=[data.TransactionOutput(address=external_address_b, value=21)],
                    nonce=11,
        ...
        with self.subTest("Transfer ETH to contract address with preset nonce"):
            ...
            self.assertEqual(
                data.UnsignedTx(
                    inputs=[data.TransactionInput(address=external_address_a, value=21)],
                    outputs=[data.TransactionOutput(address=contract_address, value=21)],
                    nonce=101,
