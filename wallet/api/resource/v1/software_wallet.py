...
class _SoftwareWallet:
    URI = "wallets/software"


class PrimaryCreator:
    URI = _SoftwareWallet.URI + "/primary/{chain_code}/create"
    ...
    def on_post(self, req, resp, chain_code):
        ...
        if wallet_manager.has_primary_wallet():
            ...
            if not name:
                count = wallet_manager.count_specific_type_wallets(chain_code, wallet_data.WalletType.SOFTWARE_PRIMARY)
...
class StandaloneImporter:
    URI = _SoftwareWallet.URI + "/standalone/{chain_code}/import"
...
class Exporter:
    URI = _SoftwareWallet.URI + "/{wallet_id}/export"
